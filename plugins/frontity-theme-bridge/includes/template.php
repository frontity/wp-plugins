...
$serverUrl = get_option('frontity_theme_bridge_settings')["serverUrl"];
$staticUrl = get_option('frontity_theme_bridge_settings')["staticUrl"];
if(!$staticUrl){
    $staticUrl = $serverUrl;
}
...
if($isStatic){
    $url = $staticUrl . $_SERVER['REQUEST_URI'];
} else {
    $url = $serverUrl . $_SERVER['REQUEST_URI'];
}
